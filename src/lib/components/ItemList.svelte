<script>
	...
	$: {
		let search = $pageState.search || '';
		// don't use $currentPage, because we don't want to call this reactively
		// when paginating
		currentPage.set(1);
		const originMatch = (item) => item.origin && item.origin.includes(search.toLowerCase());
		// filter on match either on name or on origin
		filteredItems = items.filter((item) => item.name.includes(search) || originMatch(item));
	...
	$: {
		...
		pagedItems =
			filteredItems.length > 0 ? chunk([...filteredItems], perPage)[$currentPage - 1] : [];
	}
...
{#if filteredItems.length && paginated}
	<Pagination totalItems={filteredItems.length} itemsPerPage={DEFAULT_ITEMS_PER_PAGE} />
{/if}
